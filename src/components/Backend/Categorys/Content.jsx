    ...
    useEffect(() => {
        const fetchData = async () => {
            if (isDel) {
                ...
                const respone = await categoryApi.del(del);
                setIsDel(false);
            }
            if (isStatus) {
                const data = {
                ...
                await categoryApi.update(status.id, data);
                setIsStatus(false);
            }
            const response = await categoryApi.getAll(props.params);
        ...
        <div className="col-md-10">
            <div className="card">
                <div className="card-header d-flex justify-content-between fs-1 fw-bold">
                    <div>Categorys</div>
                    <div ><Link className='btn btn-success' to={'/admin/category/create'}>Create</Link></div>
                </div>
                                ...
                                categorys?.map(e => {
                                    return (
                                        <tr key={e.id} className='p-3'>
                                            ...
                                            <td>{e.attributes.categoryName}</td>
                                            <td>
